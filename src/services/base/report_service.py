...
class ReportService:
    ...
    def _preparar_datos_nuevos(self, dataframes_por_tipo):
        """
        Toma los dataframes cargados y ejecuta todo el proceso de unión y 
        enriquecimiento para crear un reporte base.
        """
        print("🛠️  Preparando y procesando datos nuevos...")

        # 1. Preparar dataframes individuales desde los datos cargados
        ...
        if r91_df.empty:
            print("⚠️ No se encontraron datos en los archivos R91. No se puede continuar.")
            return pd.DataFrame(), pd.DataFrame() # Devuelve dos DFs vacíos

        reporte_parcial = r91_df.copy()
        print(f"📄 Base de datos nuevos creada con {len(reporte_parcial)} registros.")

        # 2. Procesar y unir todos los datos
        ...
        if not processed_vencimientos.empty:
            reporte_parcial = pd.merge(reporte_parcial, processed_vencimientos, on='Credito', how='left')
        if not analisis_df.empty:
            reporte_parcial = pd.merge(reporte_parcial, analisis_df.drop_duplicates('Credito'), on='Credito', how='left', suffixes=('', '_Analisis'))
        if not r03_df.empty:
            reporte_parcial = pd.merge(reporte_parcial, r03_df.drop_duplicates('Cedula_Cliente'), on='Cedula_Cliente', how='left', suffixes=('', '_R03'))
        if not matriz_cartera_df.empty:
            reporte_parcial['Zona'] = reporte_parcial['Zona'].astype(str).str.strip()
            matriz_cartera_df['Zona'] = matriz_cartera_df['Zona'].astype(str).str.strip()
            reporte_parcial = pd.merge(reporte_parcial, matriz_cartera_df.drop_duplicates('Zona'), on='Zona', how='left')
        ...
        if asesores_sheets:
            ...
            for item in asesores_sheets:
                if 'Codigo_Vendedor' in item["data"].columns:
                    ...
                    codigos_activos.update(codigos.dropna().unique())
            
            reporte_parcial['Codigo_Vendedor_clean'] = reporte_parcial['Codigo_Vendedor'].astype(str).str.strip().str.replace(r'\.0$', '', regex=True)
            reporte_parcial['Vendedor_Activo'] = np.where(reporte_parcial['Codigo_Vendedor_clean'].isin(codigos_activos), 'ACTIVO', 'INACTIVO')
            reporte_parcial.drop('Codigo_Vendedor_clean', axis=1, inplace=True)
    
            for item in asesores_sheets:
                info_df = item["data"]
                merge_key = item["config"]["merge_on"]
                if not info_df.empty and merge_key in reporte_parcial.columns:
                    info_df[merge_key] = pd.to_numeric(info_df[merge_key], errors='coerce').fillna(0).astype('int64').astype(str)
                    reporte_parcial[merge_key] = pd.to_numeric(reporte_parcial[merge_key], errors='coerce').fillna(0).astype('int64').astype(str)
                    reporte_parcial = pd.merge(reporte_parcial, info_df.drop_duplicates(subset=merge_key), on=merge_key, how='left')

        # 3. Aplicar todas las transformaciones y enriquecimientos
        print("⚙️  Aplicando transformaciones y cálculos...")
        reporte_parcial['Empresa'] = np.where(reporte_parcial['Tipo_Credito'] == 'DF', 'FINANSUEÑOS', 'ARPESOD')
        
        reporte_parcial = self.products_sales.assign_sales_invoice(reporte_parcial, crtmp_df)
        reporte_parcial = self.products_sales.add_product_details(reporte_parcial, crtmp_df)
        reporte_parcial = self.credit_details.enrich_credit_details(reporte_parcial, sc04_df, fnz001_df)
        reporte_parcial = self.credit_details.clean_installment_data(reporte_parcial)
        reporte_parcial = self.report_processor.map_call_center_data(reporte_parcial)
        
        reporte_parcial, negativos_fnz003 = self.report_processor.calculate_balances(reporte_parcial, fnz003_df)
        
        reporte_parcial = self.report_processor.calculate_goal_metrics(reporte_parcial, metas_franjas_df)
        reporte_parcial = self.credit_details.adjust_arrears_status(reporte_parcial)

        # 4. Unificar reporte de negativos de este proceso
        reporte_negativos = pd.DataFrame()
        lista_negativos = [df for df in [negativos_vencimientos, negativos_fnz003] if not df.empty]
        if lista_negativos:
            todos_negativos = pd.concat(lista_negativos, ignore_index=True)
            info_clientes = reporte_parcial[['Credito', 'Cedula_Cliente', 'Nombre_Cliente']].drop_duplicates(subset=['Credito'])
            reporte_negativos = pd.merge(todos_negativos[['Credito', 'Observacion']].drop_duplicates(), info_clientes, on='Credito', how='left')

        print("✅ Datos nuevos preparados y procesados.")
        return reporte_parcial, reporte_negativos
