...
class CifinController:
    def __init__(self):
        ...
        # El mapa de columnas ahora es la pieza central de esta clase
        self.column_map = {
    ...
    def process_files(self, plano_path, correcciones_path, output_path):
        """
        Orquesta el proceso completo de CIFIN. Este método será llamado por la API.
        """
        try:
            print("Controlador CIFIN: Iniciando proceso...")
            
            # 1. Cargar el archivo plano
            df_cargado = self.model.load_plano_file(plano_path)
            if df_cargado is None:
                raise ValueError("No se pudo cargar el archivo plano de CIFIN.")
            
            # 2. Iniciar el servicio con el mapa de columnas
            procesador = DataProcessorService(df_cargado, correcciones_path, self.column_map)
            df_transformado = procesador.run_all_transformations()
            
            # 3. Guardar el resultado
            self.model.df = df_transformado
            self.model.guardar_en_excel(output_path)
            
            print("Controlador CIFIN: Proceso completado exitosamente.")
            return True

        except Exception as e:
            print(f"ERROR en CifinController: {e}")
            raise e # Lanza la excepción para que la API la capture
