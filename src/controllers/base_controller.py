...
class BaseMensualController:
    def __init__(self):
        # El controlador ya no gestiona vistas de Tkinter
        print("Controlador BaseMensual inicializado.")

    def process_files(self, rutas_archivos, output_path, start_date=None, end_date=None):
        """
        Orquesta la generación del reporte consolidado.
        Este método será llamado por la API.
        """
        try:
            print("Controlador: Iniciando generación de reporte base mensual...")

            # Junta todas las rutas de archivos en una sola lista
            lista_final_rutas = []
            for lista_rutas in rutas_archivos.values():
                lista_final_rutas.extend(lista_rutas)

            if not lista_final_rutas:
                raise ValueError("No se proporcionó ningún archivo para procesar.")

            # Instancia el servicio que contiene la lógica principal
            service = ReportService(config=configuracion)
            
            print("Controlador: Generando reporte consolidado...")
            ...
            if reporte_final is None or reporte_final.empty:
                raise ValueError("El reporte final está vacío. Verifique los archivos y el rango de fechas.")
            
            print(f"Controlador: Guardando reporte en {output_path}")
            reporte_final.to_excel(output_path, index=False, sheet_name='Reporte Consolidado')
            
            return True

        except Exception as e:
            print(f"ERROR en BaseMensualController: {e}")
            raise e
